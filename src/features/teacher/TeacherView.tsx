...
export function TeacherView() {
  ...
  const closeTranscript = useCallback(() => setOpenSession(null), []);
  const {
    setDialogElement: setTranscriptDialog,
    rememberTrigger: rememberTranscriptTrigger,
  } = useModalDialog(Boolean(openSession), closeTranscript);
  const openTranscript = useCallback((session: Session, trigger: HTMLElement) => {
    rememberTranscriptTrigger(trigger);
    setOpenSession(session);
  }, [rememberTranscriptTrigger]);
  useEffect(() => { if (!hydrated) hydrate(); }, [hydrate, hydrated]);
  const meaningful = useMemo(
    () => sessions.filter((session) => session.code || session.runs.length || session.chat.length),
    [sessions],
  );
